...
describe('es6', () => {
    ...
    describe('#Dictionary', () => {
        ...
        it('работают get и set', () => {
            const dic = new core.Dictionary();
            dic.set('Moscow', 'city');
            assert.strictEqual(dic.get('Moscow'), 'city');
            assert.strictEqual(dic.get('qwerty'), false);
            assert.strictEqual(dic.set(12, 'number'), false);
            assert.strictEqual(dic.set('cow', 1), false);
            assert.strictEqual(dic.set(1, 123), false);
        });
        it('работает remove', () => {
            const dic = new core.Dictionary();
            dic.set('lana', 'singer');
            assert.strictEqual(dic.remove('lana'), true);
            assert.strictEqual(dic.remove('kizaru'), false);
        });
    });
});